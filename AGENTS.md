...
`CMakeLists.txt` configures C++20 driver; all dependencies via `cmake/Dependencies.cmake`—no ad-hoc `FetchContent`. No Git submodules—fetch via CMake for lightweight repo. Third-party setup in `cmake/deps/<Lib>.cmake`; shared helpers in `cmake/EnvyFetchContent.cmake` or `cmake/DependencyPatches.cmake`. Patches use `configure_file()` materializing scripts in binary tree via `cmake/templates/` (idempotent reconfigures). Runtime sources live in `src/`; there is no standalone public include tree. Tests under `tests/` (create when needed), mirror target names (`tests/<target>_*.cpp`). Design notes in `docs/`; update `docs/dependencies.md` when pinning/patching.

## Build, Test, and Development Commands
Configuration fixed: only `CMAKE_BUILD_TYPE` and `ENABLE_LTO` vary—no additional options. Third-party tests/examples always disabled. Dev builds: `ENABLE_LTO=OFF` for speed (re-enable for release). Configure: `cmake -S . -B out/build -G Ninja -D CMAKE_BUILD_TYPE=Release -D ENABLE_LTO=ON`. Ninja only—Makefiles break isolation. Use `./build.sh` (or `./build.bat`) for routine work; it builds `envy` plus the unit tests, so validation must run that script unless debugging a specific target. Cache third-party in `out/cache`—check before fetch. All artifacts under `out/build` (e.g., `out/build/envy`). Delete `out/build` to force rebuild; delete `out/` for pristine tree. Build driver: `cmake --build out/build --parallel` (exercises libgit2, libcurl, libssh2, mbedTLS, Lua, oneTBB, libarchive, BLAKE3). Iterate on deps: `--target <dependency>` avoids reconfigure. Tasks incomplete until `./build.sh` succeeds—no warnings/errors. Third-party configures during top-level configure, builds during top-level build—never at configure-time. macOS: `otool -L out/build/envy` must show only system libs (`libz`, `libiconv`, `libSystem`, `libresolv`, `libc++`)—third-party as dynamic deps means misconfiguration.

## Coding Style & Naming Conventions
C++20, 2-space indent (`.editorconfig`), Allman braces. STL-style naming: types/functions lowercase with underscores (`snake_case`), constants `kPascalCase`. All code lives in `envy` namespace—sub-namespaces forbidden. Initialize variables with C++20 braces (`auto x{5}`); use `=` only for assignment to existing variables. Angle-bracket includes (`<>`) only for STL/OS/standard-library; double-quotes (`""`) for third-party dependencies and first-party envy code (including `src/`). Include order: local envy headers first, then third-party headers, then STL/OS headers—separate each family with blank line. Prefer simple value types to heap-allocated smart pointers—allocate from heap only when object lifetime requires it. Inline functions in headers must be out-of-body—declare inside class, implement below (even templates). Write explicit includes and keep translation units self-contained. `clang-format` pending—run local profile, minimize diffs.
