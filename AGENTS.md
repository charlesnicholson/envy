...
C++20, 2-space indent (`.editorconfig`), Allman braces. `CamelCase` classes, `snake_case` functions, `kPascalCase` constants. Write explicit includes and keep translation units self-contained. `clang-format` pending—run local profile, minimize diffs.
