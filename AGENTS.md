...
C++20, 2-space indent (`.editorconfig`), Allman braces. `CamelCase` classes, `snake_case` functions, `kPascalCase` constants. All code lives in `envy` namespace—sub-namespaces forbidden. Angle-bracket includes (`<>`) only for STL/OS/standard-library; double-quotes (`""`) for third-party dependencies and first-party envy code (including `src/`). Write explicit includes and keep translation units self-contained. `clang-format` pending—run local profile, minimize diffs.
