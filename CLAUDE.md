...
**Debugging**: Use `ENVY_TRACE_*` macros from `trace.h` for low-level debugging—never `tui::debug` or `fprintf`. Trace output is structured JSON, controllable via `--trace-file`, and designed for production diagnostics. Add new trace event types in `trace.h` when needed.
