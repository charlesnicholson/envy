...
Envy is a freeform package manager driven by Lua scripts. "Freeform" means Envy remains unopinionated about what a package represents; authors describe packages entirely in terms of verbs exposed to Lua. Core verbs include `fetch` (acquiring sources), `deploy` (staging and installation), and `asset` (locating cached assets). The runtime ships with a cache that defaults to being user-wide so any number of projects under a user's workspace can share large payloads.
