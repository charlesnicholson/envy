...
char const *const kFixtureDir = "test_data/merge_depot";

}  // namespace

TEST_CASE("parse_depot_manifest: valid entries") {
  auto entries{
    envy::parse_depot_manifest(std::string(kFixtureDir) + "/valid_two_entries.txt")
  };
...
TEST_CASE("parse_depot_manifest: lowercases mixed-case hashes") {
  auto entries{
    envy::parse_depot_manifest(std::string(kFixtureDir) + "/mixed_case_hash.txt")
  };
...
TEST_CASE("parse_depot_manifest: skips empty lines and comments") {
  auto entries{
    envy::parse_depot_manifest(std::string(kFixtureDir) + "/comments_and_blanks.txt")
  };
...
TEST_CASE("parse_depot_manifest: skips malformed lines") {
  auto entries{
    envy::parse_depot_manifest(std::string(kFixtureDir) + "/malformed_lines.txt")
  };
...
TEST_CASE("parse_depot_manifest: handles CRLF line endings") {
  auto entries{
    envy::parse_depot_manifest(std::string(kFixtureDir) + "/crlf_endings.txt")
  };
...
TEST_CASE("parse_depot_manifest: throws on nonexistent file") {
  CHECK_THROWS_AS(envy::parse_depot_manifest("/nonexistent/file.txt"),
                  std::runtime_error);
}

TEST_CASE("parse_depot_manifest: empty file returns empty vector") {
  auto entries{
    envy::parse_depot_manifest(std::string(kFixtureDir) + "/empty.txt")
  };
...
TEST_CASE("parse_depot_manifest: preserves url paths with depot prefix") {
  auto entries{
    envy::parse_depot_manifest(std::string(kFixtureDir) + "/url_paths.txt")
  };

  REQUIRE(entries.size() == 1);
  CHECK(entries[0].path ==
        "https://cdn.example.com/depot/pkg@v1-darwin-arm64.tar.zst");
}
