...
void lua_envy_deps_install(sol::table &envy_table) {
  ...
  envy_table["asset"] = [](std::string const &identity, sol::this_state L) -> std::string {
    ...
    if (!consumer) {
      throw std::runtime_error("envy.asset: not in phase context (missing pkg)");
    }

    pkg_execution_ctx *exec_ctx{ consumer->exec_ctx };
    if (!exec_ctx) {
      throw std::runtime_error("envy.asset: missing execution context for pkg '" +
    ...
    if (!strong_reachable(consumer, identity, first_needed_by)) {
      std::string const msg{ "envy.asset: pkg '" + consumer->cfg->identity +
