...
#else
  bool result = run_check_string(f.p.get(), eng, "echo hello > /dev/null");
#endif
  CHECK(result);
}
...
TEST_CASE("run_check_string returns false for failing command") {
  ...
  bool result = run_check_string(f.p.get(), eng, "exit 1");
#else
  bool result = run_check_string(f.p.get(), eng, "false");
#endif
  CHECK_FALSE(result);
}
...
TEST_CASE("run_check_function receives project_root as directory path") {
  ...
  f.cfg->declaring_file_path = project_dir / "envy.lua";
