...
recipe::cfg recipe::cfg::parse(lua_value const &lua_val,
  ...
  if (url_it != table->end()) {  // Remote source
    ...
    result.source = builtin_source{};  // No source specified, assume builtin
  }
...
recipe::recipe(cfg cfg) : cfg_{ std::move(cfg) } {}
...
void recipe::add_dependency(recipe const *dep) { dependencies_.push_back(dep); }
